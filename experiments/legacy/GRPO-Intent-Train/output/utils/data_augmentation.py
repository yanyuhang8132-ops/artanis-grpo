...
import copy
from utils.link_load_matrix import calculate_link_load_matrix
...
def detect_forwarding_loops(G, traffic_pairs):
    ...
    """
    F = nx.DiGraph()
    for item in traffic_pairs:
        if len(item) >= 3:  # 有路径信息
            if len(item) >= 4:  # [src, dst, path, sla_bw]
                src, dst, path = item[0], item[1], item[2]
            else:  # [src, dst, path]
                src, dst, path = item
        else:  # 无路径信息，跳过
            continue
...
def calculate_load_balancing_optimized(G, traffic_pairs, load_balancing_threshold=0.1, max_paths=1, sample_ratio=0.1):
    ...
    for item in traffic_pairs:
        if len(item) >= 4:  # 新格式: [src, dst, path, sla_bw]
            src, dst, path, sla_bw = item
        elif len(item) == 3:  # 兼容格式: [src, dst, path]
            src, dst, path = item
            sla_bw = 1  # 默认值
        else:  # 旧格式: [src, dst]
            continue  # 跳过无路径信息的项
...
def calculate_interface_utilization(G, traffic_flows, interface_utilization_threshold=0.9):
...
def generate_failure_scenario(G, interface_encoding, traffic_pairs, scenario_id, ec_features, policy_data, forwarding_graphs_path=None, loop_ratio=0.1, flows_path=None, policy_path=None, adj_path=None):
    ...
    valid_pairs = []
    for item in traffic_pairs:
        if len(item) >= 4:  # [src_idx, dst_idx, path_idx, sla_bw]
            src_idx, dst_idx, path_idx, sla_bw = item
        elif len(item) == 3:  # [src_idx, dst_idx, path_idx]
            src_idx, dst_idx, path_idx = item
            sla_bw = 1  # 默认值
        else:  # [src_idx, dst_idx]
            continue  # 跳过无路径信息的项
    ...
    for item in traffic_pairs:
        ...
        if src_idx in idx_to_node and dst_idx in idx_to_node:
            traffic_flows.append((src_idx, dst_idx, path_idx, sla_bw))
    # 调用增强函数
...
def main():
    ...
    # 保存结果
    with open("augmented_dataset_all.json", "w") as f:
        f.write("[\n")  # 添加文件开头的中括号
