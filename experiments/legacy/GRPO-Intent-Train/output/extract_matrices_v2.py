...
def check_sla_satisfaction(adj_matrix, name_to_idx, src, dst, sla, k=1, max_capacity=500):
...
def simulate_acl_effect(adj_matrix, name_to_idx, acl_rules, ip_to_node):
    G = nx.Graph()
    for i in range(len(adj_matrix)):
        for j in range(len(adj_matrix)):
            if adj_matrix[i][j] > 0:
    ...
    for rule in acl_rules:
        ...
        rate = rule["rate_limit"] * 1  # 转换为 Mbps
...
def extract_and_save_matrices(jsonl_path, output_dir):
    ...
    for idx, sample in enumerate(samples):
        ...
        satisfied, paths = check_sla_satisfaction(adj_matrix.copy(), name_to_idx, src, dst, sla)
        ...
        satisfied_m, paths_m = check_sla_satisfaction(adj_matrix_modified, name_to_idx, src, dst, sla)
    ...
    # final_orig.to_csv("sla_check_results.csv", index=False)
    final_mod.to_csv("test_sla_check_results_modified.csv", index=False)
...
if __name__ == "__main__":
    extract_and_save_matrices("generated_output.jsonl", "grpo_adj_matrices")
